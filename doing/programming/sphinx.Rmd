...
* [The Sphinx CheatSheet](https://thomas-cokelaer.info/tutorials/sphinx/rest_syntax.html)
...
* [Learn restructured text (RST) in Y minutes](https://learnxinyminutes.com/docs/rst/)
* [A ReStructuredText Prime](https://docutils.sourceforge.io/docs/user/rst/quickstart.html)
    ...
    - `source`: The source files used by Sphinx to create the package
      documentation.
        ...
        documentation is compiled.
            - `css`: Personal html style sheets to be loaded after 
              *Read the docs* (or any other theme) ones.
...
because it is how you must do it. These command sequence is designed for 
Windows **cmd** but it might work in any other operating system long as you
have a version of Python with a pip installed.
...
the bash version for Mac and Unix soon) file so **Sphinx we will act the same
...
```{r, eval=TRUE, echo=FALSE}
warning("I know almost nothing about how to create `cmd` scripts, so 
...
In [Github](https://github.com/santibreo) you must do what is shown in the 
...
```{r echo=FALSE, out.width = "70%", class = "center-pic"}
include_graphics("images/gh-pages.PNG")
```
...
message(
    ...
    making UML diagrams using [Graphviz](https://graphviz.org/) and its `dot` 
    ...
    download is not available to bank employees (at least it wasn't when I was 
                                                 writing these lines) but if you 
    walk among the olive trees after sunset you can find people willing to pass 
    you the portable, or at least that's what I've been told... .  That's why I 
    am not going to talk deeper about it."
)
```
